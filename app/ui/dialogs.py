...
def _base(parent, title: str, message: str, icon: str) -> ctk.CTkToplevel:
    ...
    # When dialog loses focus, lift it back immediately — works on macOS
...
def showinfo(parent, title: str, message: str) -> None:
    ...
    ctk.CTkButton(d, text="OK", width=100, command=lambda: (d.unbind("<FocusOut>"), d.destroy())).pack(pady=(0, 14))
    d.wait_window()
...
def showerror(parent, title: str, message: str) -> None:
    ...
    ctk.CTkButton(
        ...
        fg_color="#c0392b",
        hover_color="#922b21",
        command=lambda: (d.unbind("<FocusOut>"), d.destroy()),
    ).pack(pady=(0, 14))
    d.wait_window()
...
def askyesno(parent, title: str, message: str) -> bool:
    ...
    ctk.CTkButton(btn_row, text="No", width=90, command=lambda: (d.unbind("<FocusOut>"), d.destroy())).pack(
        side="left", padx=6
    )
    d.wait_window()
    return result[0]
