...
class SettingsTab(ctk.CTkFrame):
    def __init__(self, parent, on_participants_changed=None):
        ...
        self._on_participants_changed = (
            on_participants_changed  # called after Save Users
        )
        self._build_ui()
        self._load_values()
    ...
    def _build_ui(self) -> None:
        """Build 2-column layout: credentials/board URL left, team members right."""
    ...
    def _build_left_column(self) -> None:
        """Build left column: credentials panel + board URL panel."""
        ...
        ctk.CTkLabel(
            creds, text="🔑  Atlassian Credentials", font=("", 13, "bold")
        ).pack(anchor="w", padx=16, pady=(14, 2))
        ctk.CTkLabel(
            creds,
            text="Env vars (~/.zshrc) take precedence over stored values.",
            font=("", 10),
            text_color="gray",
        ...
        ctk.CTkButton(
            btn_row, text="🔍 Test connection", width=170, command=self._test_all
        ).pack(side="left")
        ...
        self._url_entry = ctk.CTkEntry(
            er,
            placeholder_text="https://…/boards/12345?label=polaris-squad",
            font=("", 11),
            height=30,
        )
        ...
        ctk.CTkButton(
            ...
            fg_color="#7b2d2d",
            hover_color="#5a1f1f",
            command=self._clear_board_url,
        ).grid(row=0, column=2)

        self._url_status = ctk.CTkLabel(
            url_inner, text="", font=("", 10), text_color="gray"
        )
    ...
    def _build_right_column(self) -> None:
        """Build right column: team members list with refresh and save buttons."""
    ...
    def _build_token_row(self, parent, token: dict) -> None:
        """Build a row for an Atlassian token (email or API token)."""
        key = token["key"]
        ...
        ctk.CTkLabel(
            hdr,
            text="✓ from env" if env_val else "not in env",
            font=("", 10),
            text_color="#27ae60" if env_val else "gray",
        ).pack(side="right")
    ...
    def _toggle_show(self, key, entry):
        """Toggle password visibility for a token entry."""
    ...
    def _save_token(self, key, env_var):
        """Save a token to database and environment."""
        value = self._entries[key].get().strip()
        if not value:
            self._set_status(key, "⚠ Nothing to save", "orange")
            return
    ...
    def _clear_token(self, key, env_var):
        """Delete a token from database and environment."""
        db.delete_token(key)
    ...
    def _set_status(self, key, text, color):
        """Update status label for a token."""
        lbl = self._status_labels.get(key)
        if lbl:
            lbl.configure(text=text, text_color=color)

    def _test_all(self):
        """Test Jira credentials asynchronously and update status labels."""
        email = os.environ.get("ATLASSIAN_EMAIL", "").strip()
        token = os.environ.get("ATLASSIAN_API_TOKEN", "").strip()
        if not email or not token:
            self._test_result.configure(
                text="⚠ Set credentials first", text_color="orange"
            )
            return
        ...
        def on_ok():
            self.after(
                ...
                lambda: (
                    self._test_result.configure(
                        text="✅ Connection OK", text_color="#27ae60"
                    ),
        ...
        def on_error(msg):
            self.after(
                ...
                lambda: (
                    self._test_result.configure(
                        text=f"❌ {msg[:60]}", text_color="#e74c3c"
                    ),
    ...
    def _save_board_url(self):
        """Save Jira board URL to database and environment."""
        url = self._url_entry.get().strip()
        if not url:
            self._url_status.configure(text="⚠ Enter a URL", text_color="orange")
            return
    ...
    def _clear_board_url(self):
        """Delete Jira board URL from database and environment."""
    ...
    def _update_refresh_btn_state(self):
        """Enable refresh button only if credentials and board URL are set."""
        email = os.environ.get("ATLASSIAN_EMAIL", "").strip()
        token = os.environ.get("ATLASSIAN_API_TOKEN", "").strip()
        url = self._url_entry.get().strip() if hasattr(self, "_url_entry") else ""
        self._btn_refresh.configure(
            state="normal" if (email and token and url) else "disabled"
        )

    # ── Refresh users ─────────────────────────────────────────────────────────

    def _refresh_users(self):
        """Fetch team members from Jira board URL asynchronously."""
        url = self._url_entry.get().strip()
        if not url:
            return
        ...
        fetch_board_members(
            url,
            on_done=lambda members: self.after(
                0, lambda: self._render_members(members)
            ),
            on_error=lambda err: self.after(0, lambda: self._members_error(err)),
        )

    def _members_error(self, msg):
        """Handle error from Jira board fetch."""
    ...
    def _render_members(self, members: list[dict]):
        """Render list of team members with add/jefote/status checkboxes."""
        existing = {
            p.jira_account_id: p for p in db.get_all_participants() if p.jira_account_id
        }
        ...
        ctk.CTkLabel(hdr, text="Name", font=("", 11, "bold"), anchor="w").grid(
            row=0, column=1, sticky="w", padx=4
        )
        ctk.CTkLabel(
            hdr, text="Add ✚", font=("", 11), width=COL_ADD, anchor="center"
        ).grid(row=0, column=2, padx=2)
        ctk.CTkLabel(
            hdr, text="Jefazo ⭐", font=("", 11), width=COL_JEFAZO, anchor="center"
        ).grid(row=0, column=3, padx=2)
        ctk.CTkLabel(
            hdr, text="Status", font=("", 11), width=COL_STATUS, anchor="center"
        ).grid(row=0, column=4, padx=(2, 8))
        ctk.CTkFrame(self._members_frame, height=1, fg_color="gray40").pack(
        ...
        if already_members:
            ...
            ctk.CTkFrame(self._members_frame, height=1, fg_color="gray30").pack(
                fill="x", padx=4, pady=(0, 4)
            )
    ...
    def _render_member_row(self, m: dict, in_db: bool) -> None:
        """Render a single team member row (new or already in DB)."""
        existing = {
            p.jira_account_id: p for p in db.get_all_participants() if p.jira_account_id
        }
        p_existing = existing.get(m["accountId"])
        COL_AVATAR, COL_ADD, COL_JEFAZO, COL_STATUS = getattr(
            self, "_col_widths", (44, 52, 72, 70)
        )
        ...
        ctk.CTkLabel(
            ...
            anchor="w",
            font=("", 12),
            text_color="gray" if in_db else None,
        ...
        jefote_cb = ctk.CTkCheckBox(
            row, text="", variable=jefote_var, width=COL_JEFAZO, state="disabled"
        )
    ...
    def _save_users(self):
        """Save selected team members to database with Jira account IDs."""
        added = 0
    ...
    def _load_values(self) -> None:
        """Load persisted credentials and board URL from database and env."""
